...
st.markdown("""
    ...
    /* Força o container do input a ter altura reduzida */
    div[data-testid="stTextInput"] > div {
        min-height: 22px !important;
        height: 22px !important;
    }

    /* Estilo do Input Branco */
    ...
    /* Ajuste dos Títulos (Labels) para ficarem alinhados lateralmente e centralizados com o input */
    label { 
        ...
        align-items: center;
        height: 22px; 
        justify-content: flex-end; /* Alinha o texto do título à direita para encostar no campo */
        padding-right: 10px;
    }
    ...
    /* Controle do espaçamento entre as linhas do formulário */
...
def campo_horizontal(label, key, value="", on_change=None):
    # Proporção [1, 4] coloca o título em 20% da largura e o campo em 80%
    c1, c2 = st.columns([1, 4]) 
    with c1:
        st.markdown(f"<label>{label}</label>", unsafe_allow_html=True)
    with c2:
        # label_visibility="collapsed" remove o label original do topo para usarmos o lateral
...
with abas[0]:
    # Margens laterais para centralizar o formulário
    _, col_form, _ = st.columns([0.5, 3, 0.5])
    
    with col_form:
        # Renderização dos campos com título à esquerda (conforme solicitado)
        ...
        if b1.button("SALVAR ALUNO"):
            if st.session_state.nome_alu:
                aluno = {
                    ...
                    "Curso Contratado": st.session_state.curso_acumulado,
