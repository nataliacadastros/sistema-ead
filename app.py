...
st.markdown("""
    ...
    /* SUBIR ALUNOS - CONTAINER ESPECÍFICO */
    .subir-container { background-color: #1C2526; padding: 20px; border-radius: 10px; border: 1px solid #333; margin-top: 10px; }
...
with tab_subir:
    ...
    with col_input:
        ...
        u_user = sub_c1.text_area("Usuários", height=120, key="up_user")
        u_nome = sub_c2.text_area("Nome completo", height=120, key="up_nome")
        u_cell = sub_c1.text_area("Celular", height=120, key="up_cell")
        u_doc = sub_c2.text_area("Documento", height=120, key="up_doc")
        u_city = sub_c1.text_area("Cidade", height=120, key="up_city")
        u_course = sub_c2.text_area("Cursos", height=120, key="up_course")
        u_pay = sub_c1.text_area("Pagamento", height=120, key="up_pay")
        u_sell = sub_c2.text_area("Vendedor", height=120, key="up_sell")
        u_date = sub_c1.text_area("Data contrato", height=120, key="up_date")
    ...
    with col_tags:
        ...
        for curso in cursos_tag_list:
            ...
            with c_t1:
                ...
                new_tag = st.text_input(f"Tag p/ {curso}", key=f"new_{curso}", label_visibility="collapsed", placeholder="Nova tag...")
    ...
    st.write("---")
    u_file_cidades = st.file_uploader("Planilha de Códigos de Cidades", type=["xlsx"])
    
    if st.button("🚀 PROCESSAR ALUNOS", use_container_width=True):
        if not u_file_cidades or not u_user: st.error("Dados insuficientes.")
        else:
            wb_cid = load_workbook(u_file_cidades)
            ws_cid = wb_cid.active
            cod_cid = {str(row[1]).strip().upper(): str(row[2]) for row in ws_cid.iter_rows(min_row=2, values_only=True) if row[1]}
            
            l_u = u_user.strip().split('\n'); l_n = u_nome.strip().split('\n')
            l_p = u_pay.strip().split('\n'); l_c = u_course.strip().split('\n')
            ...
            for i in range(len(l_u)):
                try:
                    p_o = l_p[i].strip().upper()
                    c_o = l_c[i].strip().upper()
                    n_o = l_n[i].strip().upper() # TUDO EM MAIÚSCULO
                    fname = n_o.split(" ")[0]
                    lname = " ".join(n_o.split(" ")[1:]) if " " in n_o else ""
                    
                    t_a = [selected_tags[k] for k in cursos_tag_list if k in c_o and selected_tags.get(k)]
                    c_col = ",".join(t_a) if t_a else c_o
                    ...
                    if "CARTÃO" in p_o: pendentes.append({"Index": i, "Aluno": n_o, "Orig": p_o, "Definir": "CARTÃO"})
                    
                    processed.append({
                        "username": l_u[i], "email2": f"{l_u[i]}@profissionalizaead.com.br",
                        "name": fname, "lastname": lname, "cellphone2": l_ce[i], "document": l_d[i],
                        "city2": cod_cid.get(l_ci[i].strip().upper(), l_ci[i]), "courses": c_col, "payment": p_f,
                        "observation": obs, "ouro": "1" if "+ 10" in obs else "0", "password": "futuro", "role": "1",
                        "secretary": "MGA", "seller": l_s[i], "contract_date": l_dt[i], "active": "1"
                    })
                except: continue
    ...
    if st.session_state.get("processou") and st.session_state.get("pendentes"):
        st.warning("Confirme os pagamentos em CARTÃO:")
        edited = st.data_editor(pd.DataFrame(st.session_state.pendentes), column_config={"Definir": st.column_config.SelectboxColumn("Forma Final", options=["CARTÃO", "BOLETO"], required=True)}, disabled=["Index", "Aluno", "Orig"], hide_index=True, key="ed_pay")
        if st.button("✅ GERAR EXCEL FINAL"):
            for _, r in edited.iterrows(): st.session_state.dados_brutos[r["Index"]]["payment"] = r["Definir"]
            ...
            wb.save(out); st.download_button("📥 BAIXAR PLANILHA", out.getvalue(), f"ead_{date.today()}.xlsx")
        ...
        st.download_button("📥 BAIXAR PLANILHA", out.getvalue(), f"ead_{date.today()}.xlsx")
