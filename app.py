...
# --- CSS HUD NEON COMPLETO E ATUALIZADO (Estilo imagem_1.png) ---
st.markdown("""
    <style>
    /* Estilos Gerais */
    ...
    /* ========================================= */
    /* --- NOVOS ESTILOS ABA RELATÓRIO (HUD) --- */
    /* ========================================= */
    
    /* Novo Card de Métrica HUD */
    .card-hud-new {
    ...
    /* Barra de progresso Cidade (Estilo imagem_1.png) */
...
# --- ABA 3: RELATÓRIOS (Refatorada estilo image_1.png) ---
with tab_rel:
    df_r = safe_read()
    if not df_r.empty:
        ...
        if len(iv) == 2:
            ...
            # Gráfico Pizza: Status (Estilo Neon)
            with g1:
            ...
            # Gráfico Barra: Vendedores (Estilo Neon)
            with g2:
                st.markdown('<div class="chart-container-hud">', unsafe_allow_html=True)
                st.markdown('<p class="hud-title hud-purple">Top Vendedores</p>', unsafe_allow_html=True)
                dfv = df_f["Vendedor"].value_counts().reset_index().head(5)
                figv = px.bar(dfv, x='Vendedor', y='count', text='count')
                figv.update_traces(
                    marker_color='#bc13fe', 
                    marker_line_color='#bc13fe',
                    marker_line_width=1,
                    opacity=0.8,
            ...
            # --- SEÇÃO 3: CIDADES HUD (Barra de Progresso Única style imagem_1.png) ---
            ...
            if not df_cv.empty:
                ...
                # Barra segmentada
                segmentos_html = "".join([
                    f'<div class="hud-city-segment" style="width:{(q/t_c)*100}%; background:{cores_cidades[i%5]}; box-shadow: 0 0 10px {cores_cidades[i%5]}80;"></div>' 
                    for i, (n, q) in enumerate(df_cv.items())
                ])
                st.markdown(f'<div class="hud-city-bar-container">{segmentos_html}</div>', unsafe_allow_html=True)
                
                # Legenda HUD
                legenda_html = "".join([
                    f'<div class="legend-item"><div class="legend-color" style="background:{cores_cidades[i%5]};"></div><span style="color:{cores_cidades[i%5]}">{n} ({q})</span></div>' 
                    for i, (n, q) in enumerate(df_cv.items())
                ])
...
with tab_ger:
    ...
    if not df_g.empty:
        ...
        for _, r in df_g.iloc[::-1].iterrows():
            ...
            rows += f"<tr><td><span class='{sc}'>{r['STATUS']}</span></td><td>{r['UNID.']}</td><td>{r['TURMA']}</td><td>{r['10C']}</td><td>{r['ING']}</td><td>{r['DT_CAD']}</td><td style='color:#00f2ff;font-weight:bold;text-shadow:0 0 5px #00f2ff;'>{r['ID']}</td><td style='color:#00f2ff;font-weight:bold;text-shadow:0 0 5px #00f2ff;'>{r['ALUNO']}</td><td>{r['TEL_RESP']}</td><td>{r['TEL_ALU']}</td><td>{r['CPF']}</td><td>{r['CIDADE']}</td><td>{r['CURSO']}</td><td>{r['PAGTO']}</td><td>{r['VEND.']}</td><td>{r['DT_MAT']}</td></tr>"
