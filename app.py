...
# --- NAVEGAÇÃO ---
tab_cad, tab_ger, tab_rel, tab_subir = st.tabs(["📑 CADASTRO", "🖥️ GERENCIAMENTO", "📊 RELATÓRIOS (HUD)", "📤 SUBIR ALUNOS"])
...
# --- ABA 2: GERENCIAMENTO (MANTIDA INTEGRALMENTE) ---
with tab_ger:
...
# --- ABA 3: RELATÓRIOS (APLICANDO DESIGN HUD NEON) ---
with tab_rel:
    df_r = safe_read()
    if not df_r.empty:
        ...
        if len(iv) == 2:
            ...
            # Gráfico Pizza: Vendedores (Melhoria Agrupamento)
            with g2:
                st.markdown('<div class="chart-container-hud">', unsafe_allow_html=True)
                st.markdown('<p class="hud-title hud-purple">Top Vendedores (Agrupado)</p>', unsafe_allow_html=True)
                
                # Agrupamento por vendedor limpo
                df_v_pie = df_f['Vend_Clean'].value_counts().reset_index()
                df_v_pie.columns = ['Vendedor_Limpo', 'count']
                
                # Formata rótulo da legenda: Nome (Quantidade)
                df_v_pie['Label'] = df_v_pie.apply(lambda row: f"{row['Vendedor_Limpo']} ({row['count']})", axis=1)

                # Gráfico de Pizza (Rosca) HUD Neon
                figv = go.Figure(data=[go.Pie(
                    labels=df_v_pie['Label'], 
                    values=df_v_pie['count'], 
                    hole=0.6,
...
# --- ABA 4: SUBIR ALUNOS (MANTIDA INTEGRALMENTE) ---
with tab_subir:
