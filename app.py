...
with tab_cad:
    ...
    if os.path.exists(caminho_logo): st.image(caminho_logo, width=90)
    ...
    with centro:
        ...
        st.text_input("ID:", key=f"f_id_{s_al}")
        st.text_input("ALUNO:", key=f"f_nome_{s_al}")
        st.text_input("TEL. RESP:", key=f"f_tel_resp_{s_al}")
        st.text_input("CPF:", key=f"f_cpf_{s_al}", on_change=formatar_cpf, args=(f"f_cpf_{s_al}",))
        ...
        if st.button("💾 SALVAR ALUNO"):
            st.session_state.lista_previa.append({"ID": st.session_state[f"f_id_{s_al}"].upper(), "Aluno": st.session_state[f"f_nome_{s_al}"].upper(), "Pagto": st.session_state[f"f_pagto_{s_al}"].upper()})
            st.session_state.reset_aluno += 1
            st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)

# --- ABA 2: GERENCIAMENTO (AGGRID INTERATIVO) ---
with tab_ger:
    df_g = safe_read()
    if not df_g.empty:
...
# --- ABA 3: RELATÓRIOS (LÓGICA COMPLETA) ---
with tab_rel:
    df_r = safe_read()
    if not df_r.empty:
        ...
        if len(iv)==2:
            df_f = df_r.loc[(df_r["Data Matrícula"].dt.date >= iv[0]) & (df_r["Data Matrícula"].dt.date <= iv[1])].copy()
            total_final = sum(df_f['Pagamento'].apply(extrair_valor_geral))
            ...
            c2.markdown(f'<div class="card-hud neon-green"><span class="stat-label">ATIVOS</span><h2>{len(df_f[df_f["STATUS"]=="ATIVO"])}</h2></div>', unsafe_allow_html=True)
            c3.markdown(f'<div class="card-hud neon-red"><span class="stat-label">CANCELADOS</span><h2>{len(df_f[df_f["STATUS"]=="CANCELADO"])}</h2></div>', unsafe_allow_html=True)
            c4.markdown(f'<div class="card-hud neon-blue"><span class="stat-label">TOTAL</span><h2>R${total_final:,.2f}</h2></div>', unsafe_allow_html=True)
...
with tab_subir:
    ...
    if modo == "MANUAL":
        c1, c2 = st.columns(2)
        u_id = c1.text_area("IDs", key="in_user")
        u_nm = c2.text_area("Nomes", key="in_nome")
        if st.button("🚀 PROCESSAR DADOS"): st.success("Pronto para gerar Excel!")
