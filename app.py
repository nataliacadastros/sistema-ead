...
with tab_rel:
    try:
        ...
        if not df_r.empty:
            ...
            v_col = "Vendedor"; dt_col = "Data Matrícula"; pag_col = "Pagamento"
            ...
            if len(iv) == 2:
                ...
                df_f['v_rec'] = df_f[pag_col].apply(extrair_valor_recebido); df_f['v_tic'] = df_f[pag_col].apply(extrair_valor_geral)
    ...
    if st.button("🚀 GERAR PLANILHA EAD", use_container_width=True):
        if not os.path.exists(ARQUIVO_CIDADES): st.error("Arquivo cidades.xlsx ausente.")
            ...
            for i, item in enumerate(raw_data_final):
                ...
                processed.append({"username": item['User'], "email2": f"{item['User']}@profissionalizaead.com.br", "name": n_up.split(" ")[0], "lastname": " ".join(n_up.split(" ")[1:]) if " " in n_up else "", "cellphone2": item['Cell'], "document": item['Doc'], "city2": cid_map.get(str(item['City']).upper(), item['City']), "courses": course_f, "payment": p_f, "observation": f"{course_f} | {c_o} | {p_o}".upper(), "ouro": "1" if "10" in course_f else "0", "password": "futuro", "role": "1", "secretary": "MGA", "seller": item['Sell'], "contract_date": item['Date'], "active": "1"})
