...
st.markdown("""
    ...
    /* EXCEÇÃO LOGIN (Remove maiúsculo) */
    .login-box div[data-testid="stTextInput"] input {
        text-transform: none !important;
    }

    /* BOTÃO LOGIN ROXO / LETRA BRANCA */
    .login-box button {
    ...
    .login-box button:hover {
        background-color: #a30fdb !important;
        box-shadow: 0 0 15px rgba(188, 19, 254, 0.6) !important;
    }

    /* BOTÕES DO SISTEMA (CIANO) */
    div.stButton > button {
...
if not st.session_state.logado:
    ...
    with centro_login:
        ...
        user_in = st.text_input("USUÁRIO", key="login_final_u").strip()
        pass_in = st.text_input("SENHA", type="password", key="login_final_p").strip()
        ...
        if st.button("ENTRAR NO SISTEMA", use_container_width=True):
            ...
            if not df_users.empty:
                ...
                valido = df_users[
                    (df_users['usuario'].apply(limpar) == user_in.upper()) & 
                    (df_users['senha'].apply(limpar) == pass_in.upper())
                ]
                if not valido.empty:
...
if is_admin: tab_cad, tab_ger, tab_rel, tab_subir, tab_users = abas
else: tab_ger, tab_rel = abas
...
if is_admin:
    with tab_cad:
        ...
        with centro:
            ...
            c1.checkbox("LIB. IN-GLÊS", key=f"chk_1_{s_al}", on_change=atualizar_pagamento); c2.checkbox("CURSO BÔNUS", key=f"chk_2_{s_al}", on_change=atualizar_pagamento); c3.checkbox("CONFIRMAÇÃO", key=f"chk_3_{s_al}", on_change=atualizar_pagamento)
            st.write("")
            ...
            with b2:
                if st.button("📤 ENVIAR PLANILHA"):
                    if st.session_state.lista_previa:
                        try:
                            creds_info = st.secrets["connections"]["gsheets"]; client = gspread.authorize(Credentials.from_service_account_info(creds_info, scopes=["https://www.googleapis.com/auth/spreadsheets"])); ws = client.open_by_url(creds_info["spreadsheet"]).get_worksheet(0)
                            d_f = []
                            ...
                            ws.append_rows(d_f, value_input_option='RAW'); st.session_state.lista_previa = []; st.session_state.reset_geral += 1; st.success("Enviado com sucesso!"); st.cache_data.clear(); st.rerun()
...
with tab_rel:
    df_r = safe_read()
    if not df_r.empty:
        df_r.columns = [c.strip() for c in df_r.columns]; dt_col = "Data Matrícula"; df_r[dt_col] = pd.to_datetime(df_r[dt_col], dayfirst=True, errors='coerce'); iv = st.date_input("Filtrar Período", value=(date.today()-timedelta(days=7), date.today()))
        if len(iv) == 2:
            df_f = df_r.loc[(df_r[dt_col].dt.date >= iv[0]) & (df_r[dt_col].dt.date <= iv[1])].copy(); v_taxa = 0.0; v_cartao = 0.0; v_entrada = 0.0
            for linha in df_f['Pagamento'].tolist():
                ...
                if m_m and ("CARTÃO" in l_u or "LINK" in l_u):
                    ...
                    for v in m_f:
                        try: val = float(v.replace('.', '').replace(',', '.')); v_entrada += val if val != 50.0 else 0
                        except: pass
            total = v_taxa + v_cartao + v_entrada; c1, c2, c3, c4 = st.columns(4)
            c1.markdown(f'<div class="card-hud neon-pink"><span class="stat-label">MATRÍCULAS</span><h2>{len(df_f)}</h2></div>', unsafe_allow_html=True); c2.markdown(f'<div class="card-hud neon-green"><span class="stat-label">ATIVOS</span><h2>{len(df_f[df_f["STATUS"].str.upper()=="ATIVO"])}</h2></div>', unsafe_allow_html=True); c3.markdown(f'<div class="card-hud neon-red"><span class="stat-label">CANCELADOS</span><h2>{len(df_f[df_f["STATUS"].str.upper()=="CANCELADO"])}</h2></div>', unsafe_allow_html=True); c4.markdown(f'<div class="card-hud neon-blue"><span class="stat-label">TOTAL</span><h2>R${total:,.2f}</h2></div>', unsafe_allow_html=True)

# ABA 4: SUBIR ALUNOS (ADMIN)
if is_admin:
    with tab_subir:
        st.markdown("### 📤 IMPORTAÇÃO EAD"); modo = st.radio("Método:", ["MANUAL", "AUTOMÁTICO"], horizontal=True)
        if modo == "AUTOMÁTICO":
            df_m = safe_read()
            if not df_m.empty:
                col_f = df_m.columns[5]; df_m[col_f] = pd.to_datetime(df_m[col_f], dayfirst=True, errors='coerce'); data_sel = st.date_input("Filtrar Cadastro:", value=date.today()); df_filtrado = df_m[df_m[col_f].dt.date == data_sel]
                if not df_filtrado.empty: cids = sorted(df_filtrado[df_m.columns[11]].unique()); sel_cids = st.multiselect("Cidades:", cids); st.session_state.df_auto_ready = df_filtrado[df_filtrado[df_m.columns[11]].isin(sel_cids)]
        else:
            c1, c2 = st.columns(2); c1.text_area("IDs", key="in_user"); c1.text_area("Celulares", key="in_cell"); c1.text_area("Cidades", key="in_city"); c1.text_area("Pagamentos", key="in_pay"); c2.text_area("Nomes", key="in_nome"); c2.text_area("Documentos", key="in_doc"); c2.text_area("Cursos", key="in_cour"); c2.text_area("Vendedores", key="in_sell"); st.text_area("Datas", key="in_date")
        if st.button("🚀 PROCESSAR DADOS", use_container_width=True): st.info("Processamento ativo.")

# ABA 5: USUÁRIOS (ADMIN)
if is_admin:
    with tab_users:
        st.markdown("### 👥 GESTÃO DE ACESSOS")
        with st.form("f_users_final", clear_on_submit=True):
            nu_user = st.text_input("Novo Usuário").strip(); nu_pass = st.text_input("Senha").strip(); nu_nivel = st.selectbox("Nível", ["ADMIN", "CONSULTA"])
            if st.form_submit_button("CADASTRAR"):
                if nu_user and nu_pass:
                    try:
                        creds_info = st.secrets["connections"]["gsheets"]; client = gspread.authorize(Credentials.from_service_account_info(creds_info, scopes=["https://www.googleapis.com/auth/spreadsheets"])); ws_u = client.open_by_url(creds_info["spreadsheet"]).worksheet("usuários"); ws_u.append_row([nu_user, nu_pass, nu_nivel]); st.success(f"Usuário {nu_user} cadastrado!"); st.cache_data.clear()
                    except Exception as e: st.error(f"Erro: {e}")
        st.write("---"); st.dataframe(carregar_usuarios(), use_container_width=True, hide_index=True)

with st.sidebar:
    st.markdown(f"### 👤 {st.session_state.usuario_ativo.upper()}"); st.write(f"Nível: {st.session_state.nivel_ativo}")
